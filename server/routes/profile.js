...
router.get('/:userId', requireAuth, async (req, res) => {
  ...
  const chaptersRead = (await db.prepare(`
    SELECT COUNT(*) as c FROM reading_stats WHERE user_id = ?
  `).get(user.id)).c;

  const totalSeconds = (await db.prepare(`
  ...
  const readRows = await db.prepare('SELECT completed_at FROM reading_stats WHERE user_id = ?').all(user.id);
  ...
  if (user.favorite_book_id) {
    favoriteBook = req.user.role === 'escritor'
      ? await db.prepare('SELECT id, title, slug FROM books WHERE id = ?').get(user.favorite_book_id)
      : await db.prepare('SELECT id, title, slug FROM books WHERE id = ? AND published_at IS NOT NULL').get(user.favorite_book_id);
  }

  let favoriteChapter = null;
  if (user.favorite_chapter_id) {
    favoriteChapter = req.user.role === 'escritor'
      ? await db.prepare('SELECT id, title FROM chapters WHERE id = ?').get(user.favorite_chapter_id)
      : await db.prepare("SELECT ch.id, ch.title FROM chapters ch JOIN books b ON b.id = ch.book_id WHERE ch.id = ? AND ch.status = 'publicado' AND b.published_at IS NOT NULL").get(user.favorite_chapter_id);
  }
...
router.patch('/me', requireAuth, async (req, res) => {
  ...
  if (req.body.favorite_book_id !== undefined) {
    ...
    const validBook = id && (req.user.role === 'escritor'
      ? await db.prepare('SELECT 1 FROM books WHERE id = ?').get(id)
      : await db.prepare('SELECT 1 FROM books WHERE id = ? AND published_at IS NOT NULL').get(id));
  ...
  if (req.body.favorite_chapter_id !== undefined) {
    ...
    const validChapter = id && (req.user.role === 'escritor'
      ? await db.prepare('SELECT 1 FROM chapters WHERE id = ?').get(id)
      : await db.prepare("SELECT 1 FROM chapters ch JOIN books b ON b.id = ch.book_id WHERE ch.id = ? AND ch.status = 'publicado' AND b.published_at IS NOT NULL").get(id));
  ...
  if (req.body.avatar_url !== undefined && req.body.avatar_url !== current.avatar_url) await removeManagedUploadUrl(current.avatar_url);
