...
router.post('/login', loginIpLimiter, loginPairLimiter, async (req, res) => {
...
router.post('/logout', requireAuth, async (req, res) => {
  await db.prepare('UPDATE users SET session_version = session_version + 1 WHERE id = ?').run(req.user.id);
