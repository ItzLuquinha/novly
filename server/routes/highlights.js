...
router.post('/', requireAuth, async (req, res) => {
  const access = await getChapterAccessById(req.body?.chapter_id, req.user);
  ...
  const highlight = await db.prepare('SELECT * FROM highlights WHERE id = ?').get(result.lastInsertRowid);
  res.status(201).json({ highlight });
});

router.patch('/:id/note', requireAuth, async (req, res) => {
  const highlight = await db.prepare('SELECT * FROM highlights WHERE id = ?').get(req.params.id);
  if (!highlight || highlight.user_id !== req.user.id) return res.status(403).json({ error: 'Sem permissao.' });
  await db.prepare('UPDATE highlights SET note = ? WHERE id = ?').run(boundedString(req.body?.note, 5000, ''), req.params.id);
  res.json({ ok: true });
});
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  await db.batch([
    db.prepare("DELETE FROM likes WHERE target_type = 'highlight' AND target_id = ?").bind(highlight.id),
    db.prepare("DELETE FROM favorites WHERE target_type = 'highlight' AND target_id = ?").bind(highlight.id),
    db.prepare('DELETE FROM highlights WHERE id = ?').bind(highlight.id),
  ]);
  res.json({ ok: true });
});
