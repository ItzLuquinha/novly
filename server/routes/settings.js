...
router.patch('/email', requireAuth, async (req, res) => {
  ...
  const taken = await db.prepare('SELECT 1 FROM users WHERE email = ? AND id != ?').get(normalized, req.user.id);
  if (taken) {
    return res.status(409).json({ error: 'Este email ja esta em uso.' });
  }

  await db.prepare('UPDATE users SET email = ?, session_version = session_version + 1 WHERE id = ?').run(normalized, req.user.id);
  const refreshed = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
...
router.patch('/password', requireAuth, async (req, res) => {
  ...
  await db.prepare('UPDATE users SET password_hash = ?, session_version = session_version + 1 WHERE id = ?').run(newHash, req.user.id);
  const refreshed = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
  setSessionCookie(res, signToken(refreshed));
  res.json({ ok: true });
});

router.patch('/background', requireAuth, async (req, res) => {
  ...
  const user = await db.prepare('SELECT background_value FROM users WHERE id = ?').get(req.user.id);
  await db.prepare('UPDATE users SET background_type = ?, background_value = ? WHERE id = ?').run(background_type, backgroundValue, req.user.id);
  if (user?.background_value !== backgroundValue) await removeManagedUploadUrl(user?.background_value);
